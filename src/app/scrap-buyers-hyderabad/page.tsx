...
import ScrapGallery from "@/components/ScrapGallery";
import { SITE, telLink, whatsappLink } from "@/lib/constants";

const SLUG = "scrap-buyers-hyderabad";
        ...
        <div className="container grid items-center gap-4 lg:gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
          <div className="self-center min-w-0">
            <span className="inline-flex items-center gap-2 rounded-full bg-[#fdf8ec] px-3 py-1 text-xs font-semibold text-[#0f1c36] ring-1 ring-[#ecc257] animate-fade-up animate-float">
              <RecycleIcon className="h-3.5 w-3.5" /> Dismantling Works
            </span>
            ...
            <div className="mt-3 flex gap-2.5 max-w-xl animate-fade-up animate-fade-up-2">
              ...
              <p className="text-sm md:text-base text-[#475569]">
                End-to-end dismantling of factories, offices, hotels, and shut-down sites. Trained crew, safe removal, transport — paid against signed inventory.
              </p>
            </div>

            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-2xl">
              <Feature icon={<TagIcon />} label="Best Lot Prices" delay={1} />
              <Feature icon={<WrenchIcon />} label="Trained Dismantling Crew" delay={2} />
              <Feature icon={<TruckIcon />} label="Loading & Transport" delay={3} />
            ...
            <div className="mt-4 flex flex-wrap gap-2.5 animate-fade-up animate-fade-up-4">
              ...
              <a
                href={whatsappLink("Hi, I want a dismantling / site clearance quote.")}
          ...
          <div className="relative w-full max-w-md mx-auto">
            <div className="relative aspect-[4/3] rounded-3xl overflow-hidden bg-white shadow-xl ring-1 ring-[#ecc257]">
              <Image
                src="/images/sc1.jpg"
                alt="Dismantling works in Hyderabad"
                fill
                priority
            ...
            <div className="absolute top-3 right-3 inline-flex items-center gap-2.5 rounded-2xl bg-[#0f1c36] text-white px-3 py-2 shadow-xl ring-2 ring-accent-400/40 animate-float">
              ...
              <span className="leading-tight">
                ...
                <span className="block text-[10px] opacity-90">on Inventory Signed</span>
                <span className="block mt-1 h-0.5 w-10 bg-accent-400 rounded-full" />
              </span>
            </div>

            <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-2xl bg-white text-[#0f1c36] px-2 py-2 shadow-md ring-1 ring-ink-100">
              <FeatureChip icon={<FactoryIcon />} label="Factories & Plants" />
              <FeatureChip icon={<BuildingIcon />} label="Office Shut-Downs" />
              <FeatureChip icon={<UtensilsIcon />} label="Hotels & Restaurants" />
              <FeatureChip icon={<HandRupeeIcon />} label="On-Site Payment" />
            </div>
          </div>
        </div>
      </section>
...
function WrenchIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M14.7 6.3a4 4 0 0 0-5.4 5.4l-7 7 2 2 7-7a4 4 0 0 0 5.4-5.4l-2.7 2.7-2-2 2.7-2.7z" />
    </svg>
  );
}
function TruckIcon() {
